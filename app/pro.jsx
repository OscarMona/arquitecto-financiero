...
const Tooltip = ({text})=>{const[open,setOpen]=useState(false);return<span style={{position:"relative",display:"inline-flex",alignItems:"center"}}><button onClick={e=>{e.stopPropagation();setOpen(o=>!o);}} style={{background:"none",border:`1px solid ${C.border}`,borderRadius:"50%",width:15,height:15,fontSize:9,color:C.t3,cursor:"pointer",padding:0,lineHeight:1,display:"inline-flex",alignItems:"center",justifyContent:"center",marginLeft:4,flexShrink:0}}>?</button>{open&&<><div onClick={()=>setOpen(false)} style={{position:"fixed",inset:0,zIndex:998}}/><div style={{position:"absolute",left:0,top:20,zIndex:999,background:C.card,border:`1px solid ${C.accent}44`,borderRadius:10,padding:"10px 12px",minWidth:200,maxWidth:260,boxShadow:"0 8px 24px #000a",fontSize:11,color:C.t2,lineHeight:1.6}}>{text}<button onClick={()=>setOpen(false)} style={{display:"block",marginTop:6,background:"none",border:"none",color:C.t3,fontSize:10,cursor:"pointer",padding:0}}>Cerrar ✕</button></div></>}</span>;};
  ...
  return(
    <div style={{background:C.bg,minHeight:"100vh",color:C.t1,fontFamily:"'DM Sans',system-ui,sans-serif",paddingBottom:80}}>
      ...
      {(tab==="resumen"||tab==="registro")&&<div style={{display:"grid",gridTemplateColumns:tarjetaMes>0?`1fr 1fr 1fr 1fr 1fr${totalInvertidoActivo>0?" 1fr":""}`:`1fr 1fr 1fr 1fr${totalInvertidoActivo>0?" 1fr":""}`,gap:6,padding:"0 16px 4px"}}>
        {[
          {l:"Ingresos",v:ingMes,c:C.accent,tip:"Todo lo que entró a tu bolsillo este mes — nómina, bonos, negocios, etc."},
          {l:"Efectivo",v:efectivoMes,c:C.danger,tip:"Gastos que salieron de tu cuenta o efectivo directo, sin incluir tarjeta de crédito."},
          tarjetaMes>0?{l:"💳 Tarjeta",v:tarjetaMes,c:C.purple,tip:"Lo que llevas cargado a tarjeta este mes. Recuerda que esto lo pagarás en tu próximo corte."}:null,
          {l:"Apartado",v:ahoMes,c:C.cyan,tip:"Dinero que separaste para ahorro, metas o fondo de emergencia. No es gasto — es tuyo."},
          totalInvertidoActivo>0?{l:"📈 Inversión",v:totalInvertidoActivo,c:C.cyan,tip:"Capital activo en instrumentos de inversión. Se descuenta de tu disponible porque ya está trabajando para ti."}:null,
          {l:"Disponible",v:balMes,c:balMes>=0?C.accent:C.danger,tip:"Tu liquidez real: saldo inicial + ingresos − gastos − apartado − inversiones. Lo que tienes hoy para gastar."},
        ].filter(Boolean).map((s,i)=><div key={i} style={{background:C.card,borderRadius:10,padding:"8px 4px",textAlign:"center"}}>
          <div style={{fontSize:8,color:C.t3,display:"flex",alignItems:"center",justifyContent:"center"}}>{s.l}<Tooltip text={s.tip}/></div>
      ...
      <div style={{padding:"0 16px",animation:"fadeIn 0.3s"}}>
        ...
        {tab==="resumen"&&<div>
          ...
          {presVsReal.length>0&&<Card style={{marginBottom:12}}><div style={{fontSize:13,fontWeight:700,color:C.t1,marginBottom:10,display:"flex",alignItems:"center"}}>📊 Presupuesto vs Real<Tooltip text="Compara lo que planeaste gastar contra lo que realmente gastaste. Verde = dentro del presupuesto. Rojo = te pasaste. Sin barra = no tenías presupuesto para esa categoría."/></div>
            {(()=>{
              const ingPres=presMes["Ingresos"]||Object.entries(presMes).filter(([k])=>k.startsWith("Ingresos__")).reduce((s,[,v])=>s+(parseFloat(v)||0),0);
                ...
                <div style={{display:"flex",flexDirection:"column",gap:6}}>
                  ...
                  <button onClick={()=>{
                    ...
                    setPres(prev=>{
                      ...
                      const itemKey=`Gastos_Programados__${p.nombre}`;
                      ex[itemKey]=(parseFloat(ex[itemKey])||0)+p.monto;
                      ex["Gastos_Programados"]=(parseFloat(ex["Gastos_Programados"])||0)+p.monto;
                      up[mesTarget.key]=ex;
                      return up;
                    });
                    setProrateados(prev=>({...prev,[p.id]:true}));
          ...
          {hasBudget&&<Card style={{marginBottom:12,padding:"14px 10px"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12,paddingLeft:6}}>
              <div style={{fontSize:13,fontWeight:700,color:C.t1,display:"flex",alignItems:"center"}}>💸 Flujo de caja<Tooltip text="Muestra cómo va a evolucionar tu dinero mes a mes. La línea punteada es lo proyectado con tu presupuesto. La línea sólida es lo que realmente pasó. Si baja, estás gastando más de lo que ingresa."/></div>
        ...
        {tab==="presupuesto"&&<div>{!hasBudget?<Card style={{textAlign:"center",padding:30}}><p style={{color:C.t3}}>Aún no tienes presupuesto.</p><button onClick={()=>{setShowFirstBudget(true);setFbStep(0);setFbDraft({});}} style={{padding:"12px 24px",borderRadius:10,border:"none",background:C.accent,color:"#000",fontSize:14,fontWeight:700,cursor:"pointer",marginTop:8}}>Configurar presupuesto</button></Card>:<div>
          <div style={{fontSize:13,fontWeight:700,color:C.t1,marginBottom:4,display:"flex",alignItems:"center"}}>Presupuesto {mes} {año}<Tooltip text="Define cuánto planeas gastar en cada categoría. Los cambios se aplican de este mes en adelante. Si editas un monto y presionas 'Aplicar →', se actualiza para todos los meses siguientes."/></div><p style={{fontSize:11,color:C.t3,margin:"0 0 12px"}}>Cambia un monto y aplica de este mes en adelante</p>
        ...
        {tab==="inversiones"&&<div>
          <div style={{fontSize:13,fontWeight:700,color:C.t1,marginBottom:4,display:"flex",alignItems:"center"}}>📈 Mis inversiones<Tooltip text="Registra tus instrumentos de inversión para ver sus rendimientos en tiempo real. Al retirar, solo se registra como ingreso la ganancia — el capital vuelve a tu liquidez automáticamente."/></div>
